...
def to_rdf_node(
    val: str, namespaces: dict, lang: str | None = None, datatype: str | None = None, prefix: str | None = None
...
def rdf_transform_csv(df: pd.DataFrame, col_mapping: dict[str, str| dict], ns: dict) -> pd.DataFrame:
            ...
            for col, col_value in csv_schema.items():
                ...
                if isinstance(col_value, URIRef):
                    ...
                    if subj_col := col_value.get("subj"):
                        ...
                        subject_node = primary_node
                    if not eval(col_value.get("if", "True"), {}, {"subj": subject_node, "obj": object_node}):
                        continue  
                else:
                    continue
...
def main():
    ...
    if rdf_graph:
        logger.info("RDF graph built successfully. Serializing... (this may take a while)")
