...
async def find_entity_id(entity_label: str) -> Optional[str]:
    """
    Tool function to find the QID for a given entity label (e.g., "Guillaume Dufay").
    ...
    try:
        ...
        results = await client.wbsearchentities(entity_label, entity_type="item", limit=1)
        if results:
...
async def find_property_id(property_label: str) -> Optional[str]:
    """
    Tool function to find the PID for a given property label (e.g., "composer").
